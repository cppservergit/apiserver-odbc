...
int main()
{
	...
	s.register_webapi
	(
		webapi_path("/api/blob/test"), 
		"Upload documents to server filesystem",
		http::verb::POST, 
		{ /* inputs */
			{"title", http::field_type::STRING, true},
			{"filename.document", http::field_type::STRING, true},
			{"filename2.document", http::field_type::STRING, true}
		},
		{} /* roles */,
		[](http::request& req) 
		{
		...
		false //no security required
	);
	
	s.register_webapi
	(
		webapi_path("/api/pgsql"), 
		"Test PGSQL JSON query",
		http::verb::GET, 
		[](http::request& req) 
		{
			auto json {sql::get_json_response("DB2", "select jsonb_agg(t) from (select customerid, contactname from demo.customers) t")};
	...
	(
		...
		[](http::request& req) 
		{
			...
			auto sql {req.get_sql("sp_blob_update $blob_id, $title, $document, $filename, $content_type, $content_len")};
			sql::exec_sql("DB1", sql);
			req.response.set_body(R"({"status":"OK"})");
		}
	);
