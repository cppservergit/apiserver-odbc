...
int main()
{
	...
	s.register_webapi
	(
		webapi_path("/api/emps/view"), 
		"List of employees",
		http::verb::GET, 
		[](http::request& req) 
		{
			req.response.set_body( sql::get_json_response("DB2", "call showemp()") );
		}
	);

	s.register_webapi
	(
		webapi_path("/api/emps/get"), 
		"Retrieve employee record",
		http::verb::GET, 
		{{"empid", http::field_type::STRING, true}}, 	
		{"customer_access", "sysadmin"},
		[](http::request& req)
		{
			auto sql {req.get_sql("call getemp($empid)")};
			req.response.set_body(sql::get_json_response("DB2", sql));
		}
	);
